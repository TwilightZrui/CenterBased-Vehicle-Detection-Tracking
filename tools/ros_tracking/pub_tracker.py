  ...
  def step_centertrack(self, results, time_lag):
    ...
    if len(results) == 0:
      ...
      for det in results:
        ...
        if det['tracking_name'] not in NUSCENES_TRACKING_NAMES:
          continue 

        det['ct'] = np.array(det['translation'][:2])
        det['tracking'] = np.array(det['velocity'][:2]) * -1 * time_lag
        det['label_preds'] = NUSCENES_TRACKING_NAMES.index(det['tracking_name'])
        temp.append(det)
    ...
    max_diff = np.array([self.NUSCENE_CLS_VELOCITY_ERROR[box['tracking_name']] for box in results], np.float32)
